...
import {
  Thermometer, Moon, Wind, Heart, Shirt, ChevronDown, CheckCircle2,
  Circle, Droplets, Activity, Flame, Star, Plus, Edit2, Sparkles,
  Brain, Bone, AlertCircle, Coffee, Carrot, Fish, Leaf, Droplet,
  Phone, Shield, Smile, Frown, Zap, Sun, CloudRain, Battery, Calendar,
  TrendingUp, BookOpen, Footprints, Dumbbell, HeartPulse, Pill, Apple,
  Utensils, Bed, Clock, Award, Target, Compass, Gem, Feather, Cloud,
  Menu, User, Settings, LogOut, Bell, Search, Home, Info, HelpCircle
} from 'lucide-react';
...
/* ─────────────────────────────────────────────────────────────────────────────
   AI INSIGHT — derived from ALL symptoms, not just 2
───────────────────────────────────────────────────────────────────────────── */
function getAiInsight(symptoms, meta) {
  const { hotFlash, sleep, anxiety, mood, brainFog, fatigue, jointPain, nightSweat } = symptoms;
  ...
  if (anxiety >= 2 && mood >= 2) return {
    ...
    body: "Mood swings and anxiety are both moderate-to-severe today. The 4-7-8 breathing technique and a short outdoor walk can lower cortisol within minutes. You're not alone in this.",
  };
  if (sleep >= 2 && fatigue >= 2) return {
    title: 'Sleep & energy are taking a hit',
    body: "Poor sleep is amplifying your fatigue. Keep your bedroom at 16–18°C, avoid screens 1 hour before bed, and try magnesium glycinate supplementation after discussing with your GP.",
  };
  if (brainFog >= 2) return {
    title: 'Brain fog is notable today',
    body: 'Cognitive symptoms are common during perimenopause and menopause. Short walks, hydration, and reducing screen overload can sharpen mental clarity quickly.',
  };
...
const MENTAL_TIPS = [
  {
    id: 'breathing', icon: <Wind size={20} />, title: '4-7-8 Breathing',
    ...
    tag: 'Anxiety & Stress', color: '#7C5CBF', bg: '#F3EEFF',
  },
  {
    id: 'journaling', icon: <BookOpen size={20} />, title: 'Gratitude Journaling',
    ...
    tag: 'Low Mood', color: '#C0516A', bg: '#FFF0F3',
  },
  {
    id: 'movement', icon: <Footprints size={20} />, title: 'Gentle Movement',
    ...
    tag: 'Mood & Hot Flashes', color: '#3A8A6E', bg: '#EDFAF5',
  },
  {
    id: 'sleep', icon: <Bed size={20} />, title: 'Sleep Rituals',
    ...
    tag: 'Sleep & Night Sweats', color: '#2B6CB0', bg: '#EBF8FF',
  },
  {
    id: 'connection', icon: <Heart size={20} />, title: 'Stay Connected',
    ...
    tag: 'Loneliness', color: '#B7580A', bg: '#FFFAF0',
  },
];
...
function daysBetween(dateStr, nowStr) {
  ...
  const b = new Date(nowStr);
  return Math.floor((b - a) / 86_400_000);
}

/** Derive cycle pattern label from periods array */
function deriveCyclePattern(periods) {
  ...
  if (variance > 14) return 'Irregular';
...
function StatPill({ icon, value, label, accent, empty }) {
  return (
...
function HormoneBar({ label, value, max, color, unit = 'pmol/L' }) {
  ...
  return (
    <div className="mn-hbar-row">
      <div className="mn-hbar-top">
        ...
        {value != null
          ? <span className="mn-hbar-val" style={{ color }}>{value} <span className="mn-hbar-unit">{unit}</span></span>
...
const HORMONE_FIELDS = [
  { key: 'e2',          label: 'Oestradiol (E2)', max: 200, color: '#7C5CBF', unit: 'pmol/L' },
  { key: 'fsh',         label: 'FSH',             max: 100, color: '#E07B39', unit: 'IU/L'   },
  { key: 'lh',          label: 'LH',              max: 80,  color: '#5B6ABF', unit: 'IU/L'   },
  { key: 'progesterone',label: 'Progesterone',     max: 10,  color: '#3A8A6E', unit: 'nmol/L' },
];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 12, color: '#888', marginBottom: 20 }}>
          Copy values from your latest blood test report. Leave blank if not available.
        </p>
        {HORMONE_FIELDS.map(f => (
          <div key={f.key} style={{ marginBottom: 16 }}>
            ...
            <input
              ...
              value={draft[f.key] ?? ''}
              onChange={e => setDraft(d => ({ ...d, [f.key]: e.target.value === '' ? null : parseFloat(e.target.value) }))}
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
          <button
            ...
            style={{ flex: 1, background: accent, color: '#fff', border: 'none', borderRadius: 12, padding: '12px 0', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
          >
            Save Results
          </button>
          <button
            onClick={onClose}
            style={{ flex: 1, background: '#f0f0f0', color: '#555', border: 'none', borderRadius: 12, padding: '12px 0', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}
          >
            Cancel
          </button>
        </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <input
          ...
          placeholder="e.g. 7"
          style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1.5px solid #e0e0e0', fontSize: 14, boxSizing: 'border-box', marginBottom: 16 }}
        />

        <label style={{ fontSize: 13, color: '#555', display: 'block', marginBottom: 4 }}>
          💧 Water glasses today (out of 8)
        </label>
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (
            <button
              ...
              style={{
                ...
                borderColor: hydration >= n ? accent : '#e0e0e0',
                background: hydration >= n ? accent + '22' : '#fff',
                color: hydration >= n ? accent : '#aaa',
          ...
          <button
            onClick={() => {
              onSave({
                ...
                hydrationGlasses: hydration !== '' ? parseInt(hydration) : null,
              });
              onClose();
            }}
            style={{ flex: 1, background: accent, color: '#fff', border: 'none', borderRadius: 12, padding: '12px 0', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}
...
function CycleWeekRow({ days, onToggle, accent }) {
  return (
    <div className="mn-cycle-row">
      {days.map((d, i) => (
        <div key={i} className="mn-cycle-day" onClick={() => onToggle(i)} style={{ cursor: 'pointer' }}>
          <div
...
export default function Menopause({ setTab }) {
  ...
  const [stage, setStage] = useState(() =>
    localStorage.getItem('menopauseStage') || 'perimenopause'
  );

  const [symptoms, setSymptoms] = useState(() => {
    const saved = localStorage.getItem('menopauseSymptoms');
    return saved ? JSON.parse(saved) : {
      hotFlash: 0, nightSweat: 0, mood: 0,
      sleep: 0, brainFog: 0, jointPain: 0, anxiety: 0, fatigue: 0,
    };
  });
  ...
  const [showAllTips, setShowAllTips] = useState(() =>
    localStorage.getItem('menopauseShowAllTips') === 'true'
  );
  ...
  const [tempLog, setTempLog] = useState(() => {
    ...
    const MOCK_SEED = '[36.4,36.8,37.1,36.9,37.4,36.7,36.5]';
    if (JSON.stringify(parsed) === MOCK_SEED) {
      localStorage.removeItem('menopauseTempLog');
      return [];
    }
    return parsed;
  });
  ...
  const [hormones, setHormones] = useState(() => {
    const saved = localStorage.getItem('menopauseHormones');
    return saved ? JSON.parse(saved) : { e2: null, fsh: null, lh: null, progesterone: null };
  });

  const [sleepHydration, setSleepHydration] = useState(() => {
    const saved = localStorage.getItem('menopauseSleepHydration');
    if (saved) {
      const parsed = JSON.parse(saved);
      if (parsed.date !== todayStr()) return { sleepHours: null, hydrationGlasses: null, date: todayStr() };
      return parsed;
    }
    return { sleepHours: null, hydrationGlasses: null, date: todayStr() };
  });

  const [periods, setPeriods] = useState(() => {
    const saved = localStorage.getItem('menopausePeriods');
    return saved ? JSON.parse(saved) : [];
  });

  const [cycleWeek, setCycleWeek] = useState(() => {
    const saved = localStorage.getItem('menopauseCycleWeek');
    if (saved) {
      const parsed = JSON.parse(saved);
      if (parsed.weekStart === getWeekStart()) return parsed.days;
    }
    const days = ['M', 'T', 'W', 'T', 'F', 'S', 'S'];
    return days.map(d => ({ day: d, phase: 'none' }));
  });
  ...
  const totalLoad   = Object.values(symptoms).reduce((a, b) => a + b, 0);
  const wellnessPct = Math.max(10, 100 - Math.round((totalLoad / (SYMPTOMS.length * 3)) * 100));
  const aiInsight   = getAiInsight(symptoms, meta);
  ...
  const cyclePattern = deriveCyclePattern(sortedPeriods);
  const periodsThisYear = sortedPeriods.filter(d => d.startsWith(today.slice(0, 4))).length;

  // ─── PERSIST ─────────────────────────────────────────────────────────────
  useEffect(() => { localStorage.setItem('menopauseStage', stage); }, [stage]);
  useEffect(() => { localStorage.setItem('menopauseSymptoms', JSON.stringify(symptoms)); }, [symptoms]);
  useEffect(() => { if (moodState) localStorage.setItem('menopauseMood', JSON.stringify(moodState)); }, [moodState]);
  useEffect(() => { localStorage.setItem('menopauseChecklist', JSON.stringify(checklist)); }, [checklist]);
  useEffect(() => { localStorage.setItem('menopauseShowAllTips', showAllTips); }, [showAllTips]);
  useEffect(() => { localStorage.setItem('menopauseTempLog', JSON.stringify(tempLog)); }, [tempLog]);
  useEffect(() => { localStorage.setItem('menopauseHormones', JSON.stringify(hormones)); }, [hormones]);
  useEffect(() => { localStorage.setItem('menopauseSleepHydration', JSON.stringify(sleepHydration)); }, [sleepHydration]);
  useEffect(() => { localStorage.setItem('menopausePeriods', JSON.stringify(periods)); }, [periods]);
  useEffect(() => {
    localStorage.setItem('menopauseCycleWeek', JSON.stringify({ weekStart: getWeekStart(), days: cycleWeek }));
  }, [cycleWeek]);
  ...
  const toggleCycleDay = (index) => {
    setCycleWeek(prev => prev.map((d, i) =>
      i === index ? { ...d, phase: PHASE_CYCLE[d.phase] } : d
    ));
  };

  const markPeriodToday = () => {
    if (!periods.includes(today)) {
      setPeriods(prev => [...prev, today]);
    }
  };
  ...
  return (
    <div className="hm-root mn-root">
      ...
      {/* ── Stats Row ── */}
      <div className="mn-stats-row" onClick={() => setShowSleepModal(true)} style={{ cursor: 'pointer' }}>
        <StatPill
          icon={<Thermometer size={16} />}
          value={avgTemp ? `${avgTemp}°` : '—'}
          label="Avg Temp"
          accent={meta.accent}
          empty={!avgTemp}
        />
        <StatPill
          icon={<Flame size={16} />}
          value={`${symptoms.hotFlash !== undefined ? symptoms.hotFlash : 0}/3`}
          label="Hot Flashes"
          accent="#E07B39"
        />
        <StatPill
          icon={<Moon size={16} />}
          value={sleepHydration.sleepHours != null ? `${sleepHydration.sleepHours}h` : '—'}
          label="Sleep"
          accent="#5B6ABF"
          empty={sleepHydration.sleepHours == null}
        />
        <StatPill
          icon={<Droplets size={16} />}
          value={sleepHydration.hydrationGlasses != null ? `${sleepHydration.hydrationGlasses}/8` : '—'}
          label="Hydration"
          accent="#3A8A6E"
          empty={sleepHydration.hydrationGlasses == null}
        />
      </div>
      {(sleepHydration.sleepHours == null || sleepHydration.hydrationGlasses == null) && (
        <p style={{ fontSize: 11, color: '#aaa', textAlign: 'center', marginTop: -8, marginBottom: 4 }}>
          Tap the stats row to log sleep & hydration
        </p>
      )}
      ...
      <div className="hm-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          ...
          <button
            ...
            style={{
              background: meta.accent + '18', color: meta.accent, border: 'none',
        ...
        <p className="mn-section-sub">Enter results from your latest blood test. These guide your care team.</p>

        {HORMONE_FIELDS.every(f => hormones[f.key] == null) ? (
          <div style={{
            background: '#f9f6ff', borderRadius: 12, padding: '16px', textAlign: 'center',
            border: '1.5px dashed #d0b8f0', marginTop: 8,
          }}>
      ...
      <div className="hm-card">
        ...
        {tempLog.length === 0 ? (
          <div style={{
            background: '#f9f9f9', borderRadius: 12, padding: '16px', textAlign: 'center',
            border: '1.5px dashed #e0e0e0', marginBottom: 12,
          }}>
            <p style={{ fontSize: 13, color: '#999' }}>No temperatures logged yet. Use the input below to start tracking.</p>
          </div>
        ) : (
                  ...
                  <div className="mn-temp-bar-wrap">
                    <div
                      ...
                      style={{
                        height: `${h}px`,
                        background: isHigh
                          ? 'linear-gradient(180deg,#E05252,#F59E0B)'
                          : `linear-gradient(180deg,${meta.accent},${meta.accentMid})`,
                      }}
                    />
                  </div>
      ...
      {stage === 'perimenopause' && (
        <div className="hm-card">
          ...
          <p className="mn-section-sub">Tap a day to cycle through: none → light → period. Irregular cycles are normal during perimenopause.</p>
          ...
          <button
            ...
            style={{
              marginTop: 10, background: periods.includes(today) ? '#f0f0f0' : meta.accent,
              color: periods.includes(today) ? '#aaa' : '#fff',
              border: 'none', borderRadius: 10, padding: '8px 16px',
              fontSize: 13, fontWeight: 600, cursor: periods.includes(today) ? 'default' : 'pointer',
          ...
          <div className="mn-cycle-stats" style={{ marginTop: 14 }}>
            <div className="mn-cycle-stat">
              <p className="mn-cstat-val" style={{ color: meta.accent }}>
                {daysSincePeriod != null ? daysSincePeriod : '—'}
              </p>
              <p className="mn-cstat-lbl">Days since last period</p>
            </div>
            <div className="mn-cycle-stat">
              <p className="mn-cstat-val" style={{ color: meta.accent, fontSize: periods.length >= 2 ? undefined : 14 }}>
                {periods.length >= 2 ? cyclePattern : '—'}
              </p>
          ...
          {periods.length === 0 && (
            <p style={{ fontSize: 11, color: '#bbb', marginTop: 8 }}>
              Tap "Mark today as period start" to begin tracking. Stats will populate as you log.
            </p>
          )}
        </div>
      )}
      ...
      <div className="hm-card">
        <div className="hm-checklist-header" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <SectionLabel style={{ margin: 0 }}>DAILY WELLNESS CHECKLIST ☀️</SectionLabel>
          <ChecklistRingProgress percentage={pct} accent={meta.accent} size={60} />
        </div>
          ...
          {[
            {
              ...
              badge: 'Most Effective', color: meta.accent,
            },
            {
              ...
              badge: 'NICE Recommended', color: '#3A8A6E',
            },
            {
              ...
              badge: 'No Prescription', color: '#5B6ABF',
            },
          ].map((h, i) => (
      ...
      <div className="hm-card" style={{ background: '#FFF5F5' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button
            ...
            style={{ background: '#C0516A', color: '#fff', border: 'none', borderRadius: 20, padding: '8px 16px', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}
          >
            <Phone size={16} /> Samaritans 116 123
          </button>
          <button
            onClick={() => { window.location.href = 'tel:111'; }}
            style={{ background: '#7C5CBF', color: '#fff', border: 'none', borderRadius: 20, padding: '8px 16px', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}
          >
...
/* ─────────────────────────────────────────────────────────────────────────────
   UTILITY — get Monday of current week as YYYY-MM-DD
───────────────────────────────────────────────────────────────────────────── */
