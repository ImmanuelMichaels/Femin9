...
export default function SubscriptionPlans({ onClose, onUpgrade }) {
  ...
  const plans = billingCycle === 'monthly' ? PLANS : { free: PLANS.free, bloom: ANNUAL_PLANS.bloom, bloomPlus: ANNUAL_PLANS.bloomPlus };
  ...
  const handleUpgrade = async (planId) => {
    ...
    // Simulate payment processing (replace with actual Stripe/Paystack integration)
    ...
    if (user) {
      ...
      await updateDoc(userRef, {
        ...
        messageCount: 0 // Reset message count on upgrade
      });
    }
      ...
      {showConfetti && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ color: '#D63A6E', margin: 0 }}>Welcome to Bloom+!</h2>
            <p style={{ color: '#666' }}>You now have unlimited access to all features</p>
          </div>
        </div>
      )}
