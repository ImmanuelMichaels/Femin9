...
export default function AppHeader({ 
  onSOS, 
  onUpgrade, 
  onMenuOpen = () => {} // ✅ Default to empty function
}) {
  ...
  // Helper functions for different journey types - using safe storage
  const getCycleDay = () => {
    const lastPeriod = lsGet('lastPeriodStart', null);
    if (lastPeriod) {
  ...
  const getIVFDay = () => {
    const retrievalDate = lsGet('eggRetrievalDate', null);
    if (retrievalDate) {
  ...
  const getMenstrualPhase = () => {
    ...
    const cycleLength = parseInt(lsGet('cycleLength', '28'));
    ...
    switch(journeyType) {
      case 'pregnant':
        ...
        return [
          ...
          { color: "#E8956D", text: week && week >= 24 ? "Baby: 600g 🌽" : "Baby: Growing 🌱", tooltip: week && week >= 24 ? "Baby is the size of a scallion" : "Baby is developing rapidly" }
        ];
      case 'conceive':
        return [
  ...
  // Handle language change - using safe storage
  ...
  // ✅ Use userName from context properly
